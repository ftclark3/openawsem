...
def _loop_eoc_1(term_number,i,nres,chain_starts,chain_ends,rama_biases,p_par, p_anti, p_antihb, p_antinhb, p_parhb, a):
    """
    Refactoring a loop from _beta_efficiency_optimized into a module-scope function for parallelization
    """
...
def _beta_efficiency_optimized(oa, term_number, ssweight_file, forceGroup, k_beta):
    ...
    with ProcessPoolExecutor() as executor:
        futures = (executor.submit(_loop_eoc_1, term_number, i, nres, oa.chain_starts, oa.chain_ends, rama_biases,
...
def _pap_efficiency_optimized(oa, term_number, ssweight_file, forceGroup, k, dis_i_to_i4, pap_nu_on=True):
    ...
    for i in range(nres):
        for j in range(nres):
            resId1 = i
            chain1 = find_chain_index(resId1, oa.chain_starts, oa.chain_ends)
            resId2 = j
            chain2 = find_chain_index(resId2, oa.chain_starts, oa.chain_ends)
